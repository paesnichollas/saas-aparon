...
1. Home over-fetches data and renders two full barbershop scrollers (P0)
...
1. Home requests finished bookings even though it only renders confirmed bookings (P0)
...
1. Owner/admin large UI surfaces with heavy client hydration (P1)
...
1. `next/image` uses `fill` in multiple places without explicit `sizes` hints (P2)
...
1. Unpaginated booking payloads with deep relations (P0)
...
1. Missing composite indexes for hottest booking filters (P0)
...
1. Chat backend can return full barbershop+services sets without limits (P1)
...
1. Monthly summary aggregates in app memory from row-level data (P1)
...
1. Notification dispatch is serial inside batch (P1)
...
1. `/bookings` (owner + customer branch)
...
1. `/api/internal/notifications/dispatch`
...
1. `/api/reports/summary` + `/api/reports/monthly-summary` + `/owner/reports`
...
1. Limit and slim home listing payloads
...
1. Stop synchronous Stripe reconciliation in render paths
...
1. Add composite indexes for booking read patterns
...
1. Paginate owner bookings and avoid full relation loads
...
1. Put bounds on chat barbershop search
...
1. Reduce broad cache invalidations
...
1. Reduce admin option overfetch
...
1. Add `sizes` for all `Image fill` where missing
...
1. Normalize time-slot query keys
...
1. Changing Stripe reconciliation semantics
...
1. Aggressive caching of authenticated/role-aware pages
...
1. Large schema/index migrations without staged rollout
...
1. Decouple Stripe reconciliation from page render paths

- Impact: High
- Risk: Medium

1. Add booking composite indexes for hot filters

- Impact: High
- Risk: Medium

1. Reduce duplicate auth/session DB work in same request
...
1. Bound and slim chat search payloads

- Impact: Medium-High
- Risk: Low

1. Shrink cache invalidation fan-out (`revalidatePath`)

- Impact: Medium
- Risk: Low-Medium

1. Cut repeated admin barbershop option preloads (`pageSize: 200`)
...
1. Stabilize query keys for slot fetching

- Impact: Low-Medium
- Risk: Low

1. Tune react-query defaults where safe

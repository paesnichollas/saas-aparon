...
export async function POST(request: Request) {
  ...
  if (!barbershop) {
    return NextResponse.json(
      {
        error: "Barbearia nao encontrada ou sem permissao de edicao.",
      },
      { status: 403 },
    );
  }
  ...
  if (!ALLOWED_MIME_TYPES.has(file.type)) {
    return NextResponse.json(
      {
        error: "Formato de imagem invalido. Use PNG, JPG ou WEBP.",
      },
      { status: 400 },
    );
  }
  ...
  if (file.size < 1 || file.size > MAX_FILE_SIZE_IN_BYTES) {
    return NextResponse.json(
      {
        error: "A imagem deve ter ate 5MB.",
      },
      { status: 400 },
    );
  }
  ...
  return NextResponse.json({
    url: `/uploads/barbershops/${fileName}`,
  });
}
